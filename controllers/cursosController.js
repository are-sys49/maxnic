...
const obtenerCursos = (req, res) => {
  ...
  let query = 'SELECT t.*, s.nombre_sede as nombre_sede FROM cursos t LEFT JOIN sede s ON t.id_sede = s.id_sede';
  const params = [];

  if (sede && sede !== 'all') {
    query += ' WHERE t.id_sede = ?';
    params.push(sede);
  }
...
const obtenerSedes = (req, res) => {
  db.query('SELECT * FROM sede WHERE status = "activo"', (err, results) => {
    if (err) {
